...
def run_eval_suite(
    *,
    ...
    for case in cases:
        ...
        trace_summary["case_metadata"] = smoke_case_metadata(case)
        trace_summary["smoke_checks"] = metrics["checks"]
        results.append(
            EvaluationResult(
                case_id=case.id,
                question=case.question,
                ...
                context_precision=float(metrics["evidence_keyword_hit_rate"]),
                context_recall=float(metrics["evidence_keyword_hit_rate"]),
    ...
    run = EvaluationRun(
        ...
        score_version="smoke-evidence-v1",
        results=results,
        ...
        experiment_metadata={
            "mode": "basic_smoke_evidence_check",
            "suite_name": suite_name,
            "case_count": len(cases),
            "checks": SMOKE_CHECK_NAMES,
...
def failed_eval_result(*, case: EvaluationCase, error: Exception, latency_ms: int) -> EvaluationResult:
    ...
    return EvaluationResult(
        ...
        trace_summary={
            "status": "blocked",
            "error": message[:1000],
            "case_metadata": smoke_case_metadata(case),
            "smoke_checks": {
